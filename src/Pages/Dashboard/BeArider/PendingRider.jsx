...
import useUserRole from "../../../Hooks/useUserRole"; // ⬅️ ADD THIS

const PendingRiders = () => {
  const [selectedRider, setSelectedRider] = useState(null);
  const axiosSecure = useAxiosSecure();
  const { refetchUserRole } = useUserRole(); // ⬅️ ROLE INSTANT UPDATE
  ...
  const handleDecision = async (id, action, email) => {
    ...
    try {
      ...
      refetch(); // refresh pending riders instantly

      if (action === "approve") {
        refetchUserRole(); // 🔥 ROLE UPDATED EVERYWHERE
      }
